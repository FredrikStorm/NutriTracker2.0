...
function postMeal(date, time, location, weight, recipeId) {
    ...
    .then(data => {
        ...
        return fetch(`http://localhost:3000/tracker/user/recipe/${recipeId}`); // Fetch the recipe details
    })
    .then(response => response.json())
    .then(recipe => {
...
function editMeal(element) {
    ...
    if (weight && !isNaN(weight)) {
        ...
        .then(data => {
            ...
            return fetch(`http://localhost:3000/tracker/user/recipe/${recipeID}`);  // Hent oppdatert oppskriftsinformasjon
        })
        .then(response => response.json())
        .then(recipe => {
