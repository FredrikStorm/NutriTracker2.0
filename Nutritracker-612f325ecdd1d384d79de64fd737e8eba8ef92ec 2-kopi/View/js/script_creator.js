...
function searchIngredient() { // funksjon for å søke opp ingredienser
    ...
    const apiUrl = `http://localhost:3000/creator/foodbank/food?search=${encodeURIComponent(searchString)}`; // lager url for å søke ingredienser 
...
function addRecipeToList() {  // lagrer en fullstendig oppskrift basert på de valgte ingredienser og næringsdata 
    ...
    fetch('http://localhost:3000/creator/user/recipe', {  // post request for å lagre oppskriften i databasen 
        method: 'POST',
        headers: {
...
function loadRecipes() {    //laster inn oppskrifter fra databasen basert på userID 
    const userID = parseInt(localStorage.getItem('userID'), 10);  // henter userID 
    fetch(`http://localhost:3000/creator/user/recipe?userID=${userID}`)  // API-kall for å hente oppskrifter 
